...
def get_val_ranking(num_examples=250):
    ...
    df_exploded_labels['is_correct'] = df_exploded_labels.groupby('index').cumcount(
    ) == df_exploded_labels['label'].apply(lambda x: 0 if x == 'A' else 1)
    df_final = df_exploded_labels.drop(
        ['label', 'index'], axis=1).reset_index(drop=True)
...
def get_val_explanation():
    ...
    df = pd.DataFrame({
        ...
        'image_uncanny_description': dset['validation']['image_uncanny_description'],
...
def main():
    ...
    df_final = df_ranking.merge(df_explan,
                                on=['contest_number', 'caption'], how='left')
    df_final.loc[df_final['rank_y'].notnull(), 'rank_x'] = df_final['rank_y']
    df_final.drop(['rank_y', 'image_y', 'is_correct',
                  'winner_source'], axis=1, inplace=True)
